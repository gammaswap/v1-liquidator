...
contract CPMMShortStrategyFuzz is CPMMGammaSwapSetup {
    ...
    function testDepositReserves6x8(uint24 usdc6Amount, uint24 weth8Amount, uint24 weth8Deposit) public {
        lockProtocol();
        uint256 _usdc6Amount = uint256(usdc6Amount) * 1e6;
        uint256 _weth8Amount = uint256(weth8Amount) * 1e8;
        if(_usdc6Amount < 1000000*1e6) {
            _usdc6Amount = 1000000*1e6;
        }
        if(_weth8Amount < 1000000*1e8) {
            _weth8Amount = 1000000*1e8;
        }

        depositLiquidityInCFMMByToken(address(usdc6), address(weth8), _usdc6Amount, _weth8Amount, addr1);

        vm.startPrank(addr1);

        weth8Deposit = weth8Deposit;
        ...
        amount1 = IERC20(weth8).balanceOf(addr1) < amount1 ? IERC20(weth8).balanceOf(addr1) : amount1;
        uint256 amount0 = amount1 * _usdc6Amount / _weth8Amount;

        if(IERC20(usdc6).balanceOf(addr1) < amount0) {
            amount0 = IERC20(usdc6).balanceOf(addr1);
            amount1 = amount0 * _weth8Amount / _usdc6Amount;
        }

        uint256[] memory amountsDesired = new uint256[](2);
        amountsDesired[0] = amount0;
        amountsDesired[1] = amount1;
        ...
        if(amount0 == 0 || amount1 == 0) {
            vm.expectRevert(bytes4(keccak256("ZeroDeposits()")));
            posMgr.depositReserves(params);
        } else {
            (uint256[] memory reserves, uint256 shares) = posMgr.depositReserves(params);

            assertApproxEqRel(amountsDesired[0], reserves[0], 1e15);    // 0.1% delta
            assertApproxEqRel(amountsDesired[1], reserves[1], 1e15);    // 0.1% delta
    ...
    function testDepositReserves18x8(uint24 usdcAmount, uint24 weth8Amount, uint24 weth8Deposit) public {
        usdcAmount = usdcAmount / 10;
        ...
        depositLiquidityInCFMMByToken(address(usdc), address(weth8), uint256(usdcAmount)*1e18, uint256(weth8Amount)*1e8, addr1);
        ...
        uint256 amount0 = (amountsDesired[1] * 1e10 * usdcAmount / weth8Amount);
        amountsDesired[0] = IERC20(usdc).balanceOf(addr1) < amount0 ? IERC20(usdc).balanceOf(addr1) : amount0;

        if(amount0 != amountsDesired[0]) {
            amountsDesired[1] = (amountsDesired[0] / 1e10 * weth8Amount / usdcAmount);
        }
        ...
        uint256 reserve0 = poolData.LP_TOKEN_BALANCE * IERC20(address(usdc)).balanceOf(address(cfmm18x8)) / IERC20(cfmm18x8).totalSupply();
        uint256 reserve1 = poolData.LP_TOKEN_BALANCE * IERC20(address(weth8)).balanceOf(address(cfmm18x8)) / IERC20(cfmm18x8).totalSupply();

        amount0 = shares * reserve0 / IERC20(address(pool18x8)).totalSupply();
        amount1 = shares * reserve1 / IERC20(address(pool18x8)).totalSupply();
    ...
    function testDepositNoPull18x8(uint24 usdcAmount, uint24 weth8Amount, uint80 cfmmDeposit) public {
        lockProtocol();
        usdcAmount = usdcAmount / 10;
        weth8Amount = weth8Amount / 10;
        if(usdcAmount == 0) {
        ...
        uint256 lpTokenDeposit = IERC20(cfmm18x8).balanceOf(addr1) < uint256(cfmmDeposit) ? IERC20(cfmm18x8).balanceOf(addr1) : uint256(cfmmDeposit);
        uint256 remainingDeposit = IERC20(cfmm18x8).balanceOf(addr1) - lpTokenDeposit;
        ...
        assertEq(IERC20(cfmm18x8).balanceOf(address(pool18x8)), lpTokenDeposit + remainingDeposit);
    ...
    function testWithdrawReserves18x8(uint80 amount) public {
        lockProtocol();
        uint256 usdcAmount = 2_500_000 / 2;
        uint256 weth8Amount = 1_250 / 2;

        depositLiquidityInCFMMByToken(address(usdc), address(weth8), usdcAmount*1e18, weth8Amount*1e8, addr1);
        depositLiquidityInCFMMByToken(address(usdc), address(weth8), usdcAmount*1e18, weth8Amount*1e8, addr2);
        ...
        uint256 beforeGSLPBalance = IERC20(address(pool18x8)).balanceOf(addr1);
        uint256 gsLpAmount = beforeGSLPBalance < amount ? beforeGSLPBalance : amount;
    ...
    function testWithdrawNoPull18x8(uint80 amount) public {
        uint256 usdcAmount = 2_500_000 / 2;
        uint256 weth8Amount = 1_250 / 2;
        ...
        uint256 beforeGSLPBalance = IERC20(address(pool18x8)).balanceOf(addr1);
        uint256 beforeCfmmBalance = IERC20(address(cfmm18x8)).balanceOf(addr1);
        uint256 gsLpAmount = beforeGSLPBalance < amount ? beforeGSLPBalance : amount;
