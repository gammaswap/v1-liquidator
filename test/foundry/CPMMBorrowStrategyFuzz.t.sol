...
contract CPMMBorrowStrategyFuzz is CPMMGammaSwapSetup {
    ...
    function testIncreaseCollateral6x6(uint24 amount0, uint8 amount1, uint256 ratio0, uint256 ratio1) public {
        ...
        uint256 strikePx = uint256(tokensHeld[1]) * 1e6 / tokensHeld[0];
        uint256 expectedStrikePx = ratio[1] * 1e6 / ratio[0];

        assertApproxEqAbs(strikePx, expectedStrikePx, 1e4);

        vm.stopPrank();
    }
    function testIncreaseCollateral6x8(uint24 amount0, uint8 amount1, uint256 ratio0, uint256 ratio1) public {
        ...
        vm.startPrank(addr1);
        uint256 tokenId = posMgr.createLoan(1, cfmm6x8, addr1, 0, type(uint256).max);
        assertGt(tokenId, 0);
        ...
        uint256 beforeWeth8Balance = IERC20(weth8).balanceOf(address(pool6x8));
        uint256 beforeUSDC6Balance = IERC20(usdc6).balanceOf(address(pool6x8));

        uint128[] memory tokensHeld = posMgr.increaseCollateral(params);

        uint256 afterWeth8Balance = IERC20(weth8).balanceOf(address(pool6x8));
        uint256 afterUSDC6Balance = IERC20(usdc6).balanceOf(address(pool6x8));
        ...
        tokenId = posMgr.createLoan(1, cfmm6x8, addr2, 0, type(uint256).max);
        assertGt(tokenId, 0);
        ...
        uint256 strikePx = uint256(tokensHeld[1]) * 1e8 / tokensHeld[0];
        uint256 expectedStrikePx = ratio[1] * 1e8 / ratio[0];

        assertApproxEqAbs(strikePx, expectedStrikePx, 1e6);
    ...
    function testDecreaseCollateral6x6(uint8 amount0, uint8 amount1, uint256 ratio0, uint256 ratio1, uint8 _addr) public {
        ...
        ratio0 = bound(ratio0, 1e18, 1e19);
        ratio1 = bound(ratio1, 1e18, 1e19);
        ...
        assertEq(_amounts[0], IERC20(usdc6).balanceOf(params.to) - beforeUSDCBalanceAddr);
        assertEq(_amounts[1], IERC20(weth6).balanceOf(params.to) - beforeWethBalanceAddr);
        assertApproxEqRel(uint256(tokensHeld[1]) * 1e18 / tokensHeld[0], ratio[1] * 1e18 / ratio[0], 1e16);

        vm.stopPrank();
    }
    function testDecreaseCollateral6x8(uint8 amount0, uint8 amount1, uint256 ratio0, uint256 ratio1, uint8 _addr) public {
        depositLiquidityInCFMMByToken(address(usdc6), address(weth8), usdcAmount*1e6, wethAmount*1e8, addr1);
        depositLiquidityInCFMMByToken(address(usdc6), address(weth8), usdcAmount*1e6, wethAmount*1e8, addr2);
        ...
        uint128[] memory _amounts = new uint128[](2);
        _amounts[0] = uint128(amount0)*1e6;
        _amounts[1] = uint128(amount1)*1e8;
        ...
        uint256 beforeWethBalance = IERC20(weth8).balanceOf(address(pool6x8));
        uint256 beforeUSDCBalance = IERC20(usdc6).balanceOf(address(pool6x8));
        uint256 beforeWethBalanceAddr = IERC20(weth8).balanceOf(params.to);
        uint256 beforeUSDCBalanceAddr = IERC20(usdc6).balanceOf(params.to);
        ...
        assertEq(tokensHeld[0], ratio[0]);
        assertEq(tokensHeld[1], ratio[1]);
        assertEq(_amounts[0], beforeUSDCBalance - IERC20(usdc6).balanceOf(address(pool6x8)));
        assertEq(_amounts[1], beforeWethBalance - IERC20(weth8).balanceOf(address(pool6x8)));
        assertEq(_amounts[0], IERC20(usdc6).balanceOf(params.to) - beforeUSDCBalanceAddr);
        assertEq(_amounts[1], IERC20(weth8).balanceOf(params.to) - beforeWethBalanceAddr);
        ...
        beforeWethBalance = IERC20(weth8).balanceOf(address(pool6x8));
        beforeUSDCBalance = IERC20(usdc6).balanceOf(address(pool6x8));
        beforeWethBalanceAddr = IERC20(weth8).balanceOf(params.to);
        beforeUSDCBalanceAddr = IERC20(usdc6).balanceOf(params.to);

        tokensHeld = posMgr.decreaseCollateral(params);

        assertEq(beforeWethBalance > IERC20(weth8).balanceOf(address(pool6x8)) || beforeUSDCBalance > IERC20(usdc6).balanceOf(address(pool6x8)), true);
        assertEq(_amounts[0], IERC20(usdc6).balanceOf(params.to) - beforeUSDCBalanceAddr);
        assertEq(_amounts[1], IERC20(weth8).balanceOf(params.to) - beforeWethBalanceAddr);
        assertApproxEqRel(uint256(tokensHeld[1]) * 1e18 / tokensHeld[0], ratio[1] * 1e18 / ratio[0], 1e16);
    ...
    function testDecreaseCollateral18x8(uint8 amount0, uint8 amount1, uint256 ratio0, uint256 ratio1, uint8 _addr) public {
        ...
        uint256 beforeWethBalance = IERC20(weth8).balanceOf(address(pool18x8));
        uint256 beforeUSDCBalance = IERC20(usdc).balanceOf(address(pool18x8));
        uint256 beforeWethBalanceAddr = IERC20(weth8).balanceOf(params.to);
        uint256 beforeUSDCBalanceAddr = IERC20(usdc).balanceOf(params.to);
        ...
        assertEq(tokensHeld[0], ratio[0]);
        assertEq(tokensHeld[1], ratio[1]);
        assertEq(_amounts[0], beforeUSDCBalance - IERC20(usdc).balanceOf(address(pool18x8)));
        assertEq(_amounts[1], beforeWethBalance - IERC20(weth8).balanceOf(address(pool18x8)));
        assertEq(_amounts[0], IERC20(usdc).balanceOf(params.to) - beforeUSDCBalanceAddr);
        assertEq(_amounts[1], IERC20(weth8).balanceOf(params.to) - beforeWethBalanceAddr);
        ...
        beforeWethBalance = IERC20(weth8).balanceOf(address(pool18x8));
        beforeUSDCBalance = IERC20(usdc).balanceOf(address(pool18x8));
        beforeWethBalanceAddr = IERC20(weth8).balanceOf(params.to);
        beforeUSDCBalanceAddr = IERC20(usdc).balanceOf(params.to);

        tokensHeld = posMgr.decreaseCollateral(params);

        assertEq(beforeWethBalance > IERC20(weth8).balanceOf(address(pool18x8)) || beforeUSDCBalance > IERC20(usdc).balanceOf(address(pool18x8)), true);
        assertEq(_amounts[0], IERC20(usdc).balanceOf(params.to) - beforeUSDCBalanceAddr);
        assertEq(_amounts[1], IERC20(weth8).balanceOf(params.to) - beforeWethBalanceAddr);
        assertApproxEqAbs(uint256(tokensHeld[1]) * 1e18 / tokensHeld[0], ratio[1] * 1e18 / ratio[0], 1e12);
    ...
    function testBorrowLiquidity6x6(uint8 amount0, uint8 amount1, uint8 lpTokens, uint72 ratio0, uint72 ratio1) public {
        ...
        assertEq(params.to, posMgr.ownerOf(tokenId));
        assertGt(liquidityBorrowed,0);
        assertApproxEqRel(liquidityBorrowed, liquidity, 1e15);
        IGammaPool.LoanData memory loanData = pool6x6.loan(tokenId);
        assertEq(tokensHeld[0],loanData.tokensHeld[0]);
        assertEq(tokensHeld[1],loanData.tokensHeld[1]);
        assertApproxEqRel(ratio[0],amounts[0],1e14);
        assertApproxEqRel(ratio[1],amounts[1],1e14);
        ...
        assertEq(tokensHeld[0],loanData.tokensHeld[0]);
        assertEq(tokensHeld[1],loanData.tokensHeld[1]);
        assertApproxEqRel(_amounts[0],amounts[0],1e14);
        assertApproxEqRel(_amounts[1],amounts[1],1e14);

        if(ratio.length == 2) {
            assertApproxEqRel(uint256(loanData.tokensHeld[1]) * 1e18 / loanData.tokensHeld[0], uint256(ratio[1]) * 1e18 / ratio[0], 1e16);
        }

        vm.stopPrank();
    }
    function testBorrowLiquidity6x8(uint8 amount0, uint8 amount1, uint8 lpTokens, uint72 ratio0, uint72 ratio1) public {
        depositLiquidityInCFMMByToken(address(usdc6), address(weth8), usdcAmount*1e6, wethAmount*1e8, addr1);
        depositLiquidityInCFMMByToken(address(usdc6), address(weth8), usdcAmount*1e6, wethAmount*1e8, addr2);
        ...
        uint256[] memory _amounts = new uint256[](2);
        _amounts[0] = uint256(amount0)*1e6;
        _amounts[1] = uint256(amount1)*1e8;
        ...
        uint256[] memory ratio = new uint256[](2);
        ratio[0] = liquidity * IERC20(address(usdc6)).balanceOf(cfmm6x8) / poolData.lastCFMMInvariant;
        ratio[1] = liquidity * IERC20(address(weth8)).balanceOf(cfmm6x8) / poolData.lastCFMMInvariant;
        ...
        assertGt(liquidityBorrowed,0);
        assertApproxEqRel(liquidityBorrowed, liquidity, 1e15);
        IGammaPool.LoanData memory loanData = pool6x8.loan(tokenId);
        assertEq(tokensHeld[0],loanData.tokensHeld[0]);
        assertEq(tokensHeld[1],loanData.tokensHeld[1]);
        assertApproxEqRel(ratio[0],amounts[0],1e14);
        assertApproxEqRel(ratio[1],amounts[1],1e14);
        ...
        poolData = pool6x8.getPoolData();
        liquidity = params.lpTokens * poolData.lastCFMMInvariant / poolData.lastCFMMTotalSupply;

        if(ratio0 == ratio1) {
            ratio = new uint256[](0);
        } else {
            ratio[0] = IERC20(address(usdc6)).balanceOf(cfmm6x8) * ratio0;
            ratio[1] = IERC20(address(weth8)).balanceOf(cfmm6x8) * ratio1;
        }
        ...
        _amounts[0] = liquidity * IERC20(address(usdc6)).balanceOf(cfmm6x8) / poolData.lastCFMMInvariant;
        _amounts[1] = liquidity * IERC20(address(weth8)).balanceOf(cfmm6x8) / poolData.lastCFMMInvariant;
    ...
    function testBorrowLiquidity18x8(uint8 amount0, uint8 amount1, uint8 lpTokens, uint72 ratio0, uint72 ratio1) public {
        depositLiquidityInCFMMByToken(address(usdc), address(weth8), usdcAmount*1e18, wethAmount*1e8, addr1);
        depositLiquidityInCFMMByToken(address(usdc), address(weth8), usdcAmount*1e18, wethAmount*1e8, addr2);
        ...
        uint256[] memory ratio = new uint256[](2);
        ratio[0] = liquidity * IERC20(address(usdc)).balanceOf(cfmm18x8) / poolData.lastCFMMInvariant;
        ratio[1] = liquidity * IERC20(address(weth8)).balanceOf(cfmm18x8) / poolData.lastCFMMInvariant;
        ...
        assertGt(liquidityBorrowed,0);
        assertApproxEqAbs(liquidityBorrowed, liquidity, 1e12);
        IGammaPool.LoanData memory loanData = pool18x8.loan(tokenId);
        assertEq(tokensHeld[0],loanData.tokensHeld[0]);
        assertEq(tokensHeld[1],loanData.tokensHeld[1]);
        assertApproxEqAbs(ratio[0],amounts[0],1e2);
        assertApproxEqAbs(ratio[1],amounts[1],1e2);
        ...
        poolData = pool18x8.getPoolData();
        liquidity = params.lpTokens * poolData.lastCFMMInvariant / poolData.lastCFMMTotalSupply;

        if(ratio0 == ratio1) {
            ratio = new uint256[](0);
        } else {
            ratio[0] = IERC20(address(usdc)).balanceOf(cfmm18x8) * ratio0;
            ratio[1] = IERC20(address(weth8)).balanceOf(cfmm18x8) * ratio1;
        }
        ...
        _amounts[0] = liquidity * IERC20(address(usdc)).balanceOf(cfmm18x8) / poolData.lastCFMMInvariant;
        _amounts[1] = liquidity * IERC20(address(weth8)).balanceOf(cfmm18x8) / poolData.lastCFMMInvariant;
        ...
        assertGt(liquidityBorrowed,0);
        assertApproxEqAbs(liquidityBorrowed,liquidity, 1e12);loanData = pool.loan(tokenId);
        loanData = pool18x8.loan(tokenId);
        assertEq(tokensHeld[0],loanData.tokensHeld[0]);
        assertEq(tokensHeld[1],loanData.tokensHeld[1]);
        assertApproxEqAbs(_amounts[0],amounts[0],1e14);
        assertApproxEqAbs(_amounts[1],amounts[1],1e14);
    ...
    function testRebalanceCollateral6x6(uint96 ratio0, uint96 ratio1, bool useRatio, bool side, bool buy) public {
        ...
        if(ratio.length > 0 || deltas.length > 0) {
            ...
            if(ratio.length > 0) {
                assertApproxEqRel(uint256(tokensHeld[1]) * 1e18 / tokensHeld[0], uint256(ratio[1]) * 1e18 / ratio[0], 1e14);
            } else {
                if(deltas[0] > 0) {
                    assertEq(tokensHeld[0],loanData.tokensHeld[0] + uint256(deltas[0]));
    ...
    function testRebalanceCollateral6x8(uint96 ratio0, uint96 ratio1, bool useRatio, bool side, bool buy) public {
        lockProtocol();
        depositLiquidityInCFMMByToken(address(usdc6), address(weth8), usdcAmount*1e6, wethAmount*1e8, addr1);
        depositLiquidityInCFMMByToken(address(usdc6), address(weth8), usdcAmount*1e6, wethAmount*1e8, addr2);
        depositLiquidityInPoolFromCFMM(pool6x8, cfmm6x8, addr2);
        ...
        uint256[] memory ratio = new uint256[](2);
        ratio[0] = IERC20(address(usdc6)).balanceOf(cfmm6x8);
        ratio[1] = IERC20(address(weth8)).balanceOf(cfmm6x8);
        int256[] memory deltas = new int256[](2);
        if(useRatio && ratio0 != ratio1) {
            deltas = new int256[](0);
            ratio[0] = ratio[0] * ratio0;
    ...
    function testRebalanceCollateral18x8(uint72 ratio0, uint72 ratio1, bool useRatio, bool side, bool buy) public {
        lockProtocol();
        depositLiquidityInCFMMByToken(address(usdc), address(weth8), usdcAmount*1e18, wethAmount*1e8, addr1);
        depositLiquidityInCFMMByToken(address(usdc), address(weth8), usdcAmount*1e18, wethAmount*1e8, addr2);
        depositLiquidityInPoolFromCFMM(pool18x8, cfmm18x8, addr2);
        ...
        uint256[] memory ratio = new uint256[](2);
        ratio[0] = IERC20(address(usdc)).balanceOf(cfmm18x8);
        ratio[1] = IERC20(address(weth8)).balanceOf(cfmm18x8);
        int256[] memory deltas = new int256[](2);
        if(useRatio && ratio0 != ratio1) {
            deltas = new int256[](0);
            ratio[0] = ratio[0] * ratio0;
        ...
        if(ratio.length > 0 || deltas.length > 0) {
            ...
            IGammaPool.LoanData memory loanData1 = pool18x8.loan(tokenId);
            assertEq(loanData1.tokensHeld[0], tokensHeld[0]);
            assertEq(loanData1.tokensHeld[1], tokensHeld[1]);
